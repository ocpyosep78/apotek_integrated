...
function add_obat_alergi(id, nama, kandungan) {
    var jml = $('.alergi').length+1;
    var str = 
            '<tr class=alergi>'+
                '<td align=center>'+jml+'</td><td><input type=hidden name=id_obat[] value="'+id+'" /> '+nama+'</td><td>'+kandungan+'</td><td width=1% class=aksi align=center><a class="deletion" onclick="delete_obat(this);" title="Klik untuk hapus">&nbsp;</a></td>'+
            '</tr>';
...
function get_alergi_data_obat(id_pasien) {
    ...
    $.getJSON('models/autocomplete.php?method=get_alergi_data_obat&id_pasien='+id_pasien, function(data){
        $.each(data, function (index, value) {
            var str = 
            '<tr class=alergi>'+
                '<td align=center>'+(index+1)+'</td><td><input type=hidden name=id_obat[] value="'+value.id_barang+'" /> '+value.nama_barang+'</td><td>'+value.kandungan+'</td><td width=1% class=aksi align=center><a class="deletion" onclick="delete_obat(this);" title="Klik untuk hapus">&nbsp;</a></td>'+
            '</tr>';
            $('#data-alergi tbody').append(str);
        });
    });
...
function add_saran_pengobatan(id, nama, jumlah, keterangan) {
    ...
    var str = 
            '<tr class="saran_pengobatan tr_rows">'+
    ...
    check_alergi_obat_pasien(jml, $('#id_pasien').val(), id);
}
...
function load_last_pemeriksaan(id_pasien) {
    $.getJSON('models/autocomplete.php?method=get_last_pemeriksaan&id_pasien='+id_pasien, function(data){
        $.each(data, function (index, value) {
            var str = '<table width=45% class=data-input style="margin-left: 10px; float: right">'+
                '<tr><td width=45%>Tanggal:</td><td>'+datefmysql(value.tanggal)+'</td></tr>'+
                '<tr><td>Subjektif:</td><td>'+value.subjektif+'</td></tr>'+
                '<tr><td>Objektif:</td><td></td></tr>'+
                '<tr><td style="padding-left: 20px;">Suhu Badan:</td><td> '+value.suhu_badan+' <sup>o</sup> C</td></tr>'+
                '<tr><td style="padding-left: 20px;">Tekanan Darah:</td><td> '+value.tek_darah+' mmHg</td></tr>'+
                '<tr><td style="padding-left: 20px;">Respiration Rate:</td><td> '+value.res_rate+' x / menit</td></tr>'+
                '<tr><td style="padding-left: 20px;">Nadi:</td><td> '+value.nadi+' x / menit</td></tr>'+
                '<tr><td style="padding-left: 20px;">Gula Darah Sewaktu:</td><td> '+value.gds+' mg/dL</td></tr>'+
                '<tr><td style="padding-left: 20px;">Angka Kolesterol Total:</td><td> '+value.angka_kolesterol+' mg/dL</td></tr>'+
                '<tr><td style="padding-left: 20px;">Kadar Asam Urat:</td><td> '+value.asam_urat+' mg/dL</td></tr>'+
                '<tr><td>Assesment:</td><td>'+value.assesment+'</td></tr>'+
                '<tr><td>Goal Terapi:</td><td>'+value.goal+'</td></tr>'+
                '<tr><td>Saran Non Farmakoterapi:</td><td>'+value.saran_non_farmakoterapi+'</td></tr>'+
            '</table>';
            $('#history').append(str);
        });
    });
...
function form_add() {
    var str = '<div id=form_pemeriksaan>'+
                ...
                '<div id="tabs">'+
                    ...
                    '<div id="tabs-3">'+
                        '<div id="tabs-sub-pemeriksaan">'+
                            ...
                            '<table width=100%><tr valign=top><td width=50%>'+
                                '<table width=100% class=data-input>'+
                                    '<tr><td width=35%>Subjektif:</td><td><?= form_textarea('subjektif', NULL, 'id=subjektif cols=50') ?></td></tr>'+
                                    '<tr><td>Objektif:</td><td></td></tr>'+
                                    ...
                                    '<tr><td>Assesment:</td><td><?= form_textarea('assesment', NULL, 'id=assesment cols=50') ?></td></tr>'+
                                    '<tr><td>Goal Terapi:</td><td><?= form_textarea('goalterapi', NULL, 'id=goalterapi cols=50') ?></td></tr>'+
                                    '<tr><td>Saran Non Farmakoterapi:</td><td><?= form_textarea('sarannonfarm', NULL, 'id=sarannonfarm cols=50') ?></td></tr>'+
                                '</table>'+
                                '</td><td id="history"></td></tr></table>'+
                            '</div>'+
                            '<div id="sub-tabs-2">'+
                                '<table width=100% class=data-input>'+
                    ...
                    '<div id="tabs-4">'+
                        ...
                        '</table>'+
                        '<table width=100% class=list-data id=data-alergi>'+
                            '<thead><tr>'+
                                '<th width=3%>No.</th>'+
                                '<th width=20%>Nama Obat</th>'+
                                '<th width=75%>Kandungan</th>'+
                                '<th width=2%>#</th>'+
                            '</tr></thead>'+
                            '<tbody></tbody>'+
                        '</table>'+
    ...
    function(event,data,formated){
        ...
        load_last_pemeriksaan(data.id);
        get_alergi_data_obat(data.id);
        add_penyakit_pasien_load(data.id);
    });
    $('#cetak').click(function() {
        var id_pasien = $('#id_pasien').val();
        var rpd = ''; var rpk = ''; var ps = ''; var oh = ''; var al = ''; var dl = ''; var merokok = ''; var ko_alk = '';
        if ($('#rpd1').is(':checked') === true) {
            rpd = $('#rpd').val();
        }
        if ($('#rpk1').is(':checked') === true) {
            rpk = $('#rpk').val();
        }
        if ($('#ps1').is(':checked') === true) {
            ps = $('#ps').val();
        }
        if ($('#oh1').is(':checked') === true) {
            oh = $('#oh').val();
        }
        if ($('#al1').is(':checked') === true) {
            al = $('#al').val();
        }
        if ($('#dl1').is(':checked') === true) {
            dl = $('#dl').val();
        }
        if ($('#merokok1').is(':checked') === true) {
            merokok= $('input[name=merokok]:checked').val();
        }
        if ($('#ka1').is(':checked') === true) {
            ko_alk = $('input[name=ka]:checked').val();
        }
        ...
        window.open('pages/nota-r-penyakit.php?id_pasien='+id_pasien+'&rpd='+rpd+'&rpk='+rpk+'&ps='+ps+'&oh='+oh+'&al='+al+'&dl='+dl+'&merokok='+merokok+'&ko_alk='+ko_alk, 'Pemeriksaan Cetak', 'width='+dWidth+', height='+dHeight+', left='+x+',top='+y);
    });
    ...
    function(event,data,formated){
        ...
        $('#id_obat').val(data.id);
        add_obat_alergi(data.id, data.nama, data.kandungan);
        $('#obat, #id_obat').val('');
    });
